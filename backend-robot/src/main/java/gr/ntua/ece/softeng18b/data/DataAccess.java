...
public class DataAccess {
	...
	public Price addPrice(double value, Date mydate, long productId, long shopId) {
		//Create the new price record using a prepared statement
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			@Override
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
					PreparedStatement ps = con.prepareStatement(
							"insert into price(value, mydate, productId, shopId) values(?, ?, ?, ?)",
							Statement.RETURN_GENERATED_KEYS
							);
	...
	public List<ShowPrice> getPrices(Limits limits,String sort,String shops,String products, String tgs, String geoDist, String geoLng, String geoLat, String datefrom, String dateto) {
		double maxdist = Double.valueOf(geoDist);
		double lng = Double.valueOf(geoLng);
		double lat = Double.valueOf(geoLat);
		int count;
		count = jdbcTemplate.queryForObject("SELECT count(*) from price,product,shop,(SELECT distanceOf(shop.lng,shop.lat," + lng + ", " + lat+ ") as dist,shop.id as tempId from shop)foo where productId = product.id and shopId = shop.id and tempId=shopId and mydate>='" + datefrom + "' and mydate<='" + dateto + "'" + shops + products + tgs + " and dist < " + maxdist + "", Integer.class);
		limits.setTotal(count);
		return jdbcTemplate.query("select value, mydate, product.name, product.id, shop.id, shop.name, shop.address, dist from price,product,shop,(SELECT distanceOf(shop.lng,shop.lat," + lng + ", " + lat + ") as dist,shop.id as tempId from shop)foo where product.withdrawn = 0 and shop.withdrawn = 0 and productId = product.id and shopId = shop.id and tempId=shopId and mydate>='" + datefrom + "' and mydate<='" + dateto + "'" + shops + products + tgs + " and dist < " + maxdist + " ORDER BY " + sort + " LIMIT " + limits.getStart() + "," + limits.getCount() + "", new ShowPriceRowMapper());
	}
	
	public List<NoDistShowPrice> getPrices2(Limits limits,String sort,String shops,String products, String tgs, String datefrom, String dateto) {
		
		int count;
		count = jdbcTemplate.queryForObject("SELECT count(*) from price,product,shop where productId = product.id and shopId = shop.id and mydate>='" + datefrom + "' and mydate<='" + dateto + "'" + shops + products + tgs + "", Integer.class);
		limits.setTotal(count);
		if(limits.getCount()==-1) limits.setCount(count);
		return jdbcTemplate.query("select value, mydate, product.name, product.id, shop.id, shop.name, shop.address from price,product,shop where product.withdrawn = 0 and shop.withdrawn = 0 and productId = product.id and shopId = shop.id and mydate>='" + datefrom + "' and mydate<='" + dateto + "'" + shops + products + tgs + " ORDER BY " + sort + " LIMIT " + limits.getStart() + "," + limits.getCount() + "", new NoDistShowPriceRowMapper());
	}
